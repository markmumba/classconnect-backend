...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        if self.request.user.is_superuser:
            ...
            if self.request.user.is_school_admin:
                return CustomUser.objects.filter(school=self.request.user.school)
            else:
                return CustomUser.objects.filter(
                    school=self.request.user.school,
                    graduation_year=self.request.user.graduation_year,
                    role="student", 
                )
        else:
    ...
    def perform_create(self, serializer):
        if self.request.user.is_superuser:
            ...
            if role == "admin":
                raise PermissionDenied("School Admins cannot create other admins")
